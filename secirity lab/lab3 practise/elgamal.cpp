...
int main()
{
    ...
    // =========================================================
    // RSA ENCRYPTION / HOMOMORPHIC PROPERTY DEMO
    // =========================================================
    cout << "\n========== RSA ENCRYPTION DEMO ==========" << endl;
    ...
    // =========================================================
    cout << "\n========== RSA SIGNATURE DEMO ==========" << endl;

    // In a real RSA signature system, normally a cryptographic hash of the
    // message is signed. Here we sign an integer directly for a simple demo.
    int64 message = 12345;

    // Sign with PRIVATE key: signature = message^d mod n
    int64 signature = mod_pow(message, d, n);

    // Verify with PUBLIC key: recovered = signature^e mod n
    int64 verifiedMessage = mod_pow(signature, e, n);
    ...
    if (verifiedMessage == message)
        ...
        cout << "Signature Verification: INVALID" << endl;

    // Extra demo: change the message and show that the same signature fails
    int64 tamperedMessage = message + 1;

    cout << "\n--- Tampered Message Test ---" << endl;
    cout << "Tampered Message     = " << tamperedMessage << endl;

    if (verifiedMessage == tamperedMessage)
        cout << "Signature Verification: VALID" << endl;
    else
        cout << "Signature Verification: INVALID (message was changed)" << endl;
